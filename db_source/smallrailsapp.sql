...
CREATE TABLE IF NOT EXISTS `smallrailsapp`.`RunnerNumber` (
  `idRunnerNumber` INT NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`idRunnerNumber`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `smallrailsapp`.`UserTeam` (
  ...
  CONSTRAINT `fk_User_has_Team_User`
    FOREIGN KEY (`idUser`)
    REFERENCES `smallrailsapp`.`User` (`idUser`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_User_has_Team_Team1`
    FOREIGN KEY (`idTeam`)
    REFERENCES `smallrailsapp`.`Team` (`idTeam`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_UserTeam_RunnerNumber1`
    FOREIGN KEY (`idRunnerNumber`)
    REFERENCES `smallrailsapp`.`RunnerNumber` (`idRunnerNumber`)
    ON DELETE SET NULL
    ON UPDATE CASCADE)
ENGINE = InnoDB;
